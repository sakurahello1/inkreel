...
function characterView(c: CharRow): Character {
  return {
    ...
    voice: {
      ...
      error: c.voiceError,
      confirmed: Boolean(c.voiceSource === "minimax_system" && c.voiceId && c.voiceConfirmedAt),
      speed: c.voiceSpeed,
...
const shotInclude = {
  frame: true,
  utterances: { include: { asset: true }, orderBy: { order: "asc" as const } },
...
function shotView(s: ShotRow, extra?: { freshness?: Freshness; lineage?: Shot["lineage"] }): Shot {
  return {
    ...
    utterances: s.utterances.map((u) => ({
      id: u.id, order: u.order, kind: u.kind as "narration" | "line", characterId: u.characterId, text: u.text, tone: u.tone, emotion: u.emotion, speed: u.speed, voiceId: u.voiceId,
      status: u.status as "none" | "generating" | "ready" | "failed", error: u.error, url: assetUrl(u.asset?.path), duration: u.asset?.duration ?? u.duration ?? null, cost: u.cost,
    })),
...
export async function listProjects(): Promise<Project[]> {
  ...
  return rows.map((p) => ({
    ...
    presentStyle: p.presentStyle as Project["presentStyle"],
    narratorVoiceId: p.narratorVoiceId,
    narratorVoiceLabel: p.narratorVoiceLabel,
    narratorConfirmed: Boolean(p.narratorVoiceId && p.narratorConfirmedAt),
    kenBurns: p.kenBurns,
